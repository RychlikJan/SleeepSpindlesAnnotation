...
if __name__ == "__main__":
    ...
    #dataframe = pandas.read_csv("source64nonSpinRed4ChannelsForTrain.csv", header=None, delimiter=";")
    dataset = dataframe.values
    trainX = dataset[:, 0:dim]
    dataframe = pandas.read_csv(sys.argv[2], header=None, delimiter=";")
    #dataframe = pandas.read_csv("output64nonSpinRed4ChannelsForTrain.csv", header=None, delimiter=";")
    dataset = dataframe.values
    ...
    while 1:
        ...
        for i in range(0,trainX.shape[0],4):
            value = 0

            for j in range(trainX.shape[1]):
                value = value + abs(trainX[i][j])
                value = value + abs(trainX[i+1][j])
                value = value + abs(trainX[i+2][j])
                value = value + abs(trainX[i+3][j])
            print(value)
            if(value < border):
                print(value)
                # no spindle
        ...
        if(counter % 100 == 0):
            plt.figure()
            plt.plot(allSames)
            plt.title("Same in time-4 Channels")
            plt.show()

            plt.figure()
            plt.plot(allDiff)
            plt.title("Diff in time -4 Channels")
            plt.show()
        if(border > 250):
            break
